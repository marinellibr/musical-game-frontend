...
@Component({ selector: 'app-theme-card', template: `<article class="theme-card" [style.--theme-accent]="accent()"><span class="theme-category">{{ theme().category || theme().type }}</span><h1>{{ theme().title }}</h1>@if (showExample() && theme().example) { <p>{{ theme().example }}</p> }</article>` })
