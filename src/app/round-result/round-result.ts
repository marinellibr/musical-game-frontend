...
export class RoundResult implements OnInit {
  ...
  constructor() {
    effect(() => {
      ...
      if (status === 'THEME_REVEAL') void this.router.navigate(gameRoute(this.roomCode, 'theme'));
      if (status === 'GAME_RESULTS') void this.router.navigate(gameRoute(this.roomCode, 'game-result'));
    });
  }
  ngOnInit(): void {
