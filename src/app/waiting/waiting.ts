...
export class Waiting implements OnInit, OnDestroy {
  ...
  ngOnInit(): void { this.roomCode = (this.route.snapshot.paramMap.get('roomCode') || '').toUpperCase(); const session = this.rooms.sessionFor(this.roomCode); if (session) this.rooms.connect(session); else void this.router.navigate(['/room', this.roomCode]); this.timer = setInterval(() => this.now.set(Date.now()), 1000); }
